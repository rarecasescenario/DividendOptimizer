...
//    private final CopyOnWriteArrayList<SymbolCurrentState> csl = new CopyOnWriteArrayList<>();
    ...
    // main
    public List<SymbolCurrentState> getData(List<String> symbolList) throws InterruptedException, ExecutionException {
	...
	for (int y = 0; y < futureList.size(); y++) {
	    ...
	    System.out.printf("%-8S | %6.2f | %5.2f | %6.2f | %n", p.getSymbol(), p.getPrice(), p.getChangedPercent(),
		    p.getMarketCap());
	}
